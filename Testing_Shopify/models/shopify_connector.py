                    ...
                    for customer in customers:
                        if customer is not None:
                            ...
                            if not odoo_customer:
                                name = ""
                                if customer.attributes['first_name'] is None:
                                    customer.attributes['first_name'] = ""
                                if customer.attributes['last_name'] is None:
                                    customer.attributes['last_name'] = ""

                                if customer.attributes['last_name']:
                ...
                if orders:
                    for order in orders:
                        ...
                        if 'customer' in order.attributes:
                            ...
                            if order.attributes['customer'].attributes['first_name'] is None:
                                order.attributes['customer'].attributes['first_name'] = ""
                            if order.attributes['customer'].attributes['last_name'] is None:
                            ...
                            if not all_products_fields_available:
                                continue
                            date = datetime.strptime(order.attributes['created_at'], "%Y-%m-%dT%H:%M:%S-%f:%y")
                            ...
                            print("order_imported: ", order_imported)
                            for item in order.attributes['line_items']:
                                product_id = item.attributes['product_id']
                                if product_id:
            ...
            for product in products:
                if shopify.Product.exists(product.shopify_product_id):
                    ...
                    if product.image_1024:
                        ...
                        binary_in = base64.b64decode(product.image_1024)
...
class Dropboxlinks(models.Model):
    ...
    no_of_orders_sync = fields.Integer('SO Imported', readonly=True)
    no_of_orders_sync_update = fields.Integer('SO Updated', readonly=True)
    no_of_products_sync = fields.Integer('P. Exported', readonly=True)
    no_of_products_sync_update = fields.Integer('P. Updated', readonly=True)
    no_of_customers_sync = fields.Integer('C. Imported', readonly=True)
    no_of_customers_sync_update = fields.Integer('C. Updated', readonly=True)
