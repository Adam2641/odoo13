        ...
        if self.env.user.token:
            try:
                ...
                if inbox_id:
                    inbox_id = inbox_id[0]
                    response = requests.get(
                        'https://graph.microsoft.com/v1.0/me/mailFolders/' + inbox_id + '/messages?$top=1000&$count=true',
                        headers={
                            'Host': 'outlook.office.com',
                            'Authorization': 'Bearer {0}'.format(self.env.user.token),
                            'Accept': 'application/json',
    ...
    def generate_refresh_token(self):
        ...
        if self.env.user.refresh_token:
            ...
            if 'access_token' not in response:
                response["error_/opt/odoo13/custom_addons/odoo_xerodescription"] = response["error_description"].replace("\\r\\n", " ")
