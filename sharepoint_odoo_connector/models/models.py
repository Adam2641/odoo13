...
class sharepointlinks(models.Model):
    ...
    order_id = fields.Many2one('res.partner', string='Partner Reference', required=True,
...
class sharepointupload(models.Model):
    ...
    upload_file_data_sharepoint = fields.Many2many('ir.attachment', 'sharepoint_ir_attachments_rel',
                                                   'sharepoint_id', 'attachment_id', 'Attachments')
    downloaded_file_data_sharepoint = fields.Many2many('ir.attachment', 'downloaded_ir_attachments_rel',
                                                       'downloaded_id', 'attachment_id', 'Attachments')
                        ...
                        if is_list:
                            ...
                            for each in self.upload_file_data_sharepoint:
                                ...
                                downloadable_link = sp_obj.download_file_link(sharepoint_site_name,
                                                                              sharepoint_document_name,
                                                                              attach_file_name, self.name, file_path)
                            ...
                            query = "SELECT attachment_id FROM sharepoint_ir_attachments_rel WHERE sharepoint_id="+\
                                    str(self.id)+""
        ...
        if sharepoint_email and sharepoint_url and sharepoint_pwd:
            if self._uid == self.user_name_sharepoint.create_uid.id:
                ...
                if connection_flag:
                    ...
                    raise ValidationError('Connection Failed! Please check your SharePoint credentials.')
            else:
                raise ValidationError('Authentication Failed! Please verify your SharePoint account.')
        else:
            raise ValidationError('Connection Failed!SharePoint credentials are missing.')
