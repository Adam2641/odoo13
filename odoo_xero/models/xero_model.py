...
class InvoicesSync(models.Model):
        ...
        for invoice in invoices:
            ...
            if not partner_id or not invoice_line_ids:
                ...
                if contact_id:
                    ...
                    if invoice_date:
                        odoo_invoice_date = invoice.date_invoice
                    ...
                    for each_invoice_line in invoice_line_ids:
                        ...
                        s_taxes = each_invoice_line.invoice_line_tax_ids
    ...
    def update_invoice_status(self, xero, invoice):
        ...
        if invoice.state:
            ...
            if invoice.state == 'paid':
                ...
                pass
        if not xero_invoice['Status'] == 'AUTHORISED' and invoice.state == 'paid':
                ...
                for line_item in xero_invoice['LineItems']:
                    try:
                        ...
                        payment_dict = {
                            ...
                            "Invoice": {"InvoiceID": xero_invoice['InvoiceID']},
                            # "Account": {"Code": xero_invoice['AccountCode']},
                            "Account": {"Code": line_item['AccountCode']},
                            "Date": datetime.strptime(str(self.payment_ids[-1].payment_date), '%Y-%m-%d'),
                            "Amount": line_item['LineAmount'] + line_item['TaxAmount']
                            # "Amount": self.payment_ids[-1].amount
                        ...
                        raise UserError(
                            ("Please enable payments on account having code " + str(line_item['AccountCode'])) + " in xero")
    ...
    #     value = super(InvoicesSync, self)._compute_amount()
    #
    #     print(value)
    #     return value
...
class Xero_Import(models.Model):
    ...
    login_url = fields.Char('Login URL', readonly=True)
    credentials = fields.Char(string='Credentials')
            ...
            for xero_contact in xero_all_contacts:
                ...
                if not self.env['res.partner'].search([('xero_contact_id', '=', str(xero_contact['ContactID']))]):
                    ...
                    self.env['res.partner'].create({
                        ...
                        'phone': phone_no,
                        'mobile': mob,
                        'customer': xero_contact['IsCustomer'] if 'IsCustomer' in xero_contact else None,
                        'supplier': xero_contact['IsSupplier'] if 'IsSupplier' in xero_contact else None,
            ...
            try:
                for xero_invoice in xero_all_invoices:
                    if not self.env['account.invoice'].search(
                        ...
                        invoice_number_uniq = self.env['account.invoice'].search(
                        ...
                        # if not 'DueDateString' in xero_invoice:
                        due_date = str((xero_invoice['Date'] - timedelta(days=30)).date())

                        if state == 'authorised':
                            invoice = self.env['account.invoice'].create({
                                'xero_invoice_id': xero_invoice['InvoiceID'], 'type': inv_type,
                                'number': xero_invoice['InvoiceNumber'],
                                'currency_id': currency_id.id,

                                'residual': xero_invoice['AmountDue'],
                                'partner_id': customer.id,
                                'date_invoice': xero_invoice['DateString'],
                                'date_due': due_date,
                                ...
                                if "AccountCode" in line_item:
                                    ...
                                    if 'TaxType' in line_item:
                                        ...
                                        invoice_line = self.env['account.invoice.line'].create({
                                            ...
                                            'price_unit': line_item['UnitAmount'] if 'UnitAmount' in line_item else None,
                                            'account_id': account_id,
    ...
    def export_all_invoices(self):
        self.env['account.invoice'].sync_invoices_to_xero()